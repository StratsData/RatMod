[![GitHub stars](https://img.shields.io/github/stars/MonPseudo/SuperMod?style=social)](https://github.com/MonPseudo/SuperMod/stargazers)

#Rat Mod

A Simple Mod That Fuck Minecraft Accounts 🔍

# IT WILL BE ADDED SOON THE .JAR

# DISCLAIMER

I am not responsible for your actions.
This is for educational purposes.
Use it to understand, not to do malicious things.


AND THE V1 SUCK YOU NEED TO WAIT V2 IT WILL BE MORE ADVANCE TRUST ME

# Setup

You Need to have JDK 21 👋

Download [Here](https://adoptium.net/fr/temurin/releases)

Then You Need to Install Eclipse Or Intellj Idea

Open a project with the folder. 🌐

Then Change.

And Build.

If i explain bad you can see tutorial on youtube 😴

# Feature Of V1.0.0

Discord Webhook - Not Obfuscated

Get Session token of the Minecraft Account

Detect IF PREMIUM OR CRACKED ACC.

Get the IP, ISP AND SYSTEM INFO

WITH THE COUNTRY 🌍


# Contributors

Feel free to pull a request, it make me so happy that people are interesed in this project.
